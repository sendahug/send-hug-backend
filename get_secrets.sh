...
for s in $(gcloud --project=send-a-hug-platform secrets list --filter="labels.type:json" | cut -f1 -d" " | sed 1d)
do
	if [[ -f "$CREDS_DIR/$s/latest.json" ]]; then
		...
		gcloud --project=send-a-hug-platform secrets versions access "latest" --secret="$s" --format='get(payload.data)' | tr '_-' '/+' | base64 -d > $CREDS_DIR/$s/latest.json
	fi
done
